...
function buildElkGraph(module) {
    ...
    // EZSchem patch: enable ELK partitioning when any child has a partition
    var hasPartitions = children.some(function (c) {
        return c.layoutOptions &&
            c.layoutOptions['org.eclipse.elk.partitioning.partition'];
    });
    if (hasPartitions) {
        children.forEach(function (c) {
            if (!c.layoutOptions) c.layoutOptions = {};
            if (!c.layoutOptions['org.eclipse.elk.partitioning.partition']) {
                c.layoutOptions['org.eclipse.elk.partitioning.partition'] = '0';
            }
        });
    }
    var graph = {
